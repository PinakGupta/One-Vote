...
const UserVoteResults = () => {
  const { electionId } = useParams(); // Get electionId from route
  const [candidates, setCandidates] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [resultsVisible, setResultsVisible] = useState(false);
  ...
  useEffect(() => {
    const checkResultsVisibility = async () => {
      ...
      try {
        // Optionally make this election-specific if backend supports it
        const response = await axios.get(`${server}/admin/results-visibility`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    const fetchVoteCounts = async () => {
      ...
      try {
        const response = await axios.get(`${server}/election/${electionId}/vote-count`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        console.error("Error fetching vote counts:", error);
        setError(error.response?.data?.message || "Failed to load vote counts. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
