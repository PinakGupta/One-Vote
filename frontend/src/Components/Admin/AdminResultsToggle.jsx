...
const AdminResultsToggle = () => {
  const { id, electionId } = useParams(); // Extract id (adminId) and electionId
  const [showResults, setShowResults] = useState(false);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [message, setMessage] = useState("");
  ...
  useEffect(() => {
    const fetchResultsVisibility = async () => {
      try {
        if (!id || !electionId) {
          setError("Admin ID or Election ID not found.");
          setLoading(false);
          return;
        }
        ...
        const response = await axios.get(`${server}/admin/election/${electionId}/results-visibility`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });

        console.log("Results Visibility Response:", response.data);

        if (response.data.success) {
          setShowResults(response.data.data.showResults);
        } else {
          setError("Failed to load current settings.");
        }
      } catch (error) {
        console.error("Error fetching results visibility:", error);
        setError(error.response?.data?.message || "Failed to load current settings.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleToggleResults = async () => {
    try {
      ...
      const response = await axios.patch(
        `${server}/admin/election/${electionId}/toggle-results`,
        { showResults: !showResults },
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="w-full flex justify-center">
      <div className="w-2/3 mx-auto bg-gray-900 shadow-lg rounded-lg p-8 my-6 border border-gray-700">
        <h2 className="text-3xl font-bold mb-6 text-white">
          Election Results Visibility (Election ID: {electionId})
        </h2>
        {loading ? (
          <p className="text-gray-400 text-lg">Loading...</p>
        ) : (
          <>
            <div className="flex items-center justify-between mb-6">
              <p className="text-white text-lg">
                Currently, election results for ID {electionId} are{" "}
                <span className={showResults ? "text-green-400" : "text-red-400"}>
                  {showResults ? "visible" : "hidden"}
                </span>{" "}
                to voters.
            ...
            {message && (
              <div className="mt-6 p-4 bg-green-900 text-green-200 rounded text-lg">{message}</div>
            )}
            {error && (
              <div className="mt-6 p-4 bg-red-900 text-red-200 rounded text-lg">{error}</div>
            )}
            <div className="mt-8 p-6 border border-gray-700 rounded bg-gray-800">
              <h3 className="text-xl font-semibold mb-4 text-white">Note:</h3>
              <p className="text-gray-300 text-lg">
                When results are visible, all voters will be able to see the current vote counts for
                each candidate in this election. When hidden, only administrators can view the results.
              </p>
            </div>
          </>
        )}
