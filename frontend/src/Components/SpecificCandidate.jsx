...
import { toogleVoted } from '../Redux/slicer';
...
function SpecificCandidate() {
   ...
   const getSpecificCandidateDetails = async () => {
      try {
         const token = localStorage.getItem('accessToken');
         if (!token) {
            setErr('Authorization token missing. Please log in again.');
            return;
         }
         ...
         setErr(error.response?.data?.message || 'Please log in again to access this route.');
      }
   };
   ...
   const performVote = async () => {
      try {
         ...
         const response = await axios.post(
            `${server}/candidates/candidate-list/${candidateId}`,
            { candidateId },
            {
               headers: {
                  Authorization: `Bearer ${token}`,
         ...
         setErr(err.response?.data?.message || 'Please log in again to perform voting.');
      }
   };
            ...
            <div className="main-candidate-container flex-col flex gap-20 p-8 w-full">
               ...
               <div className="section-1 grid sm:grid-cols-3 gap-6">
                  ...
                  <div className="candidate-basic justify-center text-left flex gap-3 flex-col">
                     ...
                     <p className="subheading text-2xl md:text-xl lg:text-2xl">
                        {`Representative: `}
                        <strong>{candidateData.representative}</strong>
                     </p>
                     <p className="subheading text-2xl md:text-xl lg:text-2xl">
                        {`Date of Birth: `}
                     ...
                     <p className="subheading text-2xl md:text-xl lg:text-2xl">
                        {`Candidate: `}
                        <strong>{candidateData.candidateType}</strong>
                     </p>
                     <p className="subheading text-2xl md:text-xl lg:text-2xl">
                        {`Belongs To: `}
                        <strong>{candidateData.town}</strong>
                     </p>
                  </div>
                  {!isVoted ? (
                     <div className="vote-button flex justify-center items-center">
               ...
               <div className="section-2 flex gap-10 flex-col">
                  <p className="text-2xl md:text-3xl lg:text-4xl font-medium">Promises</p>
                  <div className="promises p-4">
                     {candidateData.promise &&
                        candidateData.promise.map((item, index) => (
                           <div className="promiseList" key={index}>
                              <p className="text-xl md:text-2xl lg:text-3xl">{`~ ${item}`}</p>
                              <br />
                           </div>
