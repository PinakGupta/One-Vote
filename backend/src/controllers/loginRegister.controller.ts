...
export const register = async (req: Request, res: Response) => {
   try {
      const { firstName, lastName, uniqueId, voterId, password, role } = req.body;

      // Validate required fields
      if (!uniqueId || !password || (role !== "admin" && (!firstName || !lastName || !voterId))) {
         throw new ApiError(400, 'Required fields are mandatory');
      }

      // Check for existing user or admin by uniqueId
      const existsUser = await User.findOne({ uniqueId });
      const existsAdmin = await Admin.findOne({ uniqueId });
      ...
      if (role !== "admin") {
         const isCandidate = await Candidate.findOne({ uniqueId, voterId });
         if (isCandidate) {
